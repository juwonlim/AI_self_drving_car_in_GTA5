...
from data_collection.navigation_img_process import grab_screen #네비게이션 용도
...
#아래의 detect_lane함수는 아직까지 이 파일내에서 이미지 함수들을  호출하고 있기에 수정해야함
''' 
def detect_lane(screen):
    if screen is None or screen.size == 0:
        print("[ERROR] screen is None or empty.")
        return [[], []], []

    image = crop(screen)
    if image is None or image.size == 0:
        print("[ERROR] crop() returned None or empty.")
        return [[], []], []
    ...
    try:
        ...
        print(f"[ERROR] inRange failed: {e}")
        return [[], []], []

    mask = cv2.bitwise_or(mask_white, mask_yellow)
    # HSV 마스크 시각화
    cv2.imshow("MASK", mask)
    print("Image shape mask:", mask.shape)   # canny 적용 후 크기 확인
    cv2.namedWindow("MASK", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("MASK", 640, 360) #창 크기
    cv2.waitKey(1)
    ...
    image = grayscale(image)
    if image is None or image.size == 0:
        print("[ERROR] grayscale failed.")
        return [[], []], []

    image = gaussian_blur(image, 7)
    if image is None or image.size == 0:
        print("[ERROR] gaussian_blur failed.")
        return [[], []], []
    ...
    print("Image shape Canny:", image.shape)   # canny 적용 후 크기 확인
    cv2.namedWindow("Canny", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Canny", 640, 360) #창 크기
    cv2.waitKey(1)

    # 화면의 중간~아래 쪽 영역만 추출
    #image = region_of_interest(image, np.array([[(200, 250), (200, 630), (1100, 630), (1100, 250)]], np.int32)) #내가 잘라낸 운전영역 900x380--차량 내부시점 기준인데 이제는 바이크라
    #image = region_of_interest(image,np.array([[(0, 150), (0, 600), (1280, 600), (1280, 150)]], np.int32)) #바이크 뷰 기준에서 상단 150~600px 영역만 사용하는 ROI로, 노란선 위쪽 도로 영역만 정확히 포함. 
                                                                                                           #1280x720 해상도 기준
    image = region_of_interest(image, np.array([[(0, 0), (0, 450), (1280, 450), (1280, 0)]], np.int32)) # crop된 이미지와 같은 사이즈 이미지만 한번 사용해봄
  
   # 필요하다면 아래 사다리꼴로 다시 지정 --> 못미더워서 그냥 주석처리
   #image = region_of_interest(image, np.array([[(100, 450), (500, 300), (780, 300), (1180, 450)]], np.int32))


    # ROI 영역 시각화
    cv2.imshow("ROI", image)
   #cv2.imwrite("roi_debug.png", image) #추가해서 저장해서 확인용도
    print("Image shape after ROI:", image.shape)   # ROI 적용 후 크기 확인
    cv2.namedWindow("ROI", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("ROI", 640, 360) #창 크기
    cv2.waitKey(1)
    ...
    if image is None or image.size == 0:
        print("[ERROR] region_of_interest failed.")
        return [[], []], []

    lines = hough_lines(image)
    lanes,stop_line = construct_lane(lines)
    return  (lanes,stop_line),image  #2개의 tuple값, data_collect.py에서 (lane, stop_line), lane_img = detect_lane(screen)이 코드로 받음.
...
def detect_lane(screen):
    ...
    if 'canny_edge_lines' in processed:
        ...
        cv2.namedWindow("Canny", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("Canny", 640, 360)
        cv2.waitKey(1)
...
def main():
    while True:
        ...
        #image = region_of_interest(image, np.array([[(0, 380), (300, 300), (700, 300), (900, 380)]], np.int32)) #위의 image(900x380사이즈)를 받아 사다리꼴 생성--> 자동차 내부시점기준--이제는 바이크라
        #image = region_of_interest(image,np.array([[(0, 150), (0, 600), (1280, 600), (1280, 150)]], np.int32))
