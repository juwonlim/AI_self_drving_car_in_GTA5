...
def crop(image):
    ...
    #위쪽은 하늘 제거, 아래쪽은 차량 대시보드 제거
    #return image[280:-130, :, :]
    #return image[250:630, 200:1100]  # ROI 전제에 맞춤 (y: 250~630, x: 200~1100)
    return image[240:640, :, :]  # 예: 높이 720 기준으로 중간 부분 400픽셀 확보
    #h, w = image.shape[:2]
    #if h < 630 or w < 1100:
     #   print(f"[ERROR] Image too small to crop: {w}x{h}")
      #  return None
    #return image[250:630, 200:1100]
...
#shape에러 관련 방어코드 추가된 함수
#당분간 none이 발생하는 에러떄문에 주석처리하고 개선된 함수사용
''' 
def detect_lane(screen):
    if screen is None:
        return [[], []], []
    ...
    image = cv2.bitwise_and(image, image, mask=mask)  # 색상 필터링된 이미지 생성
    ...
    image = region_of_interest(image, np.array([[
        (0, 360), (0, 720), (1280, 720), (1280, 360)]], np.int32))
...
def detect_lane(screen):
    ...
    # 화면의 중간~아래 쪽 영역만 추출
#    image = region_of_interest(image, np.array([[(0, 360), (0, 720), (1280, 720), (1280, 360)]], np.int32))
    image = region_of_interest(image, np.array([[(200, 250), (200, 630), (1100, 630), (1100, 250)]], np.int32)) #내가 잘라낸 운전영역 900x380
...
def main():
    while True:
        ...
        image = region_of_interest(image, np.array([[(0, 380), (300, 300), (700, 300), (900, 380)]], np.int32)) #위의 image(900x380사이즈)를 받아 사다리꼴 생성
