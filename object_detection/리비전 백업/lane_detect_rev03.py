...
def construct_lane(lines):
    ...
    #return lane, stop_line  #lane은 딕셔너리가 아니라 튜플임, 그런데 메인함수에서 'lane["lanes"]' 이렇게 호출해서 이건 TypeError: tuple indices must be integers or slices 발생 가능성이 큼.
    return {"lanes": lane, "stop_line": stop_line} #이렇게 바꿔야 main에 맞음
...
""" 
 #실제 복자한 알고리즘 수행함수가 아니며 hough함수와 contruct_lane함수 호출용도 --- 불필요한 함수
def detect_lane():
    #roi = get_preprocessed()  # preprocess.py의 get_preprocessed는 이제 roi만 반환함
    ...
    return lane, stop_line #차선과 정지선 추출
...
def main():
    ...
    cropped_roi = roi[crop_top:crop_bottom, :, :]
    ...
    while True:
        ...
        #lane, stop_line = detect_lane() #불필요한 함수
