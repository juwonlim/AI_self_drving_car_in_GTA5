...
def preprocess_img(masked):
    ...
    roi_debug = draw_roi_polygon(roi_debug, roi_vertices[0], color=(0, 0, 255), thickness=5)
   

    #roi_debug_resized = cv2.resize(roi_debug, (426, 240))
    #cv2.imshow("ROI Debug Polygon", roi_debug_resized)
    #cv2.imshow("ROI Debug Polygon", roi_debug) #자꾸 polygon이 안그려져서 resize안하고 테스트
    cv2.namedWindow("ROI Debug Polygon", cv2.WINDOW_NORMAL)
     
    cv2.imshow("ROI Debug Polygon", roi_debug)
    print("ROI Debug shape:", roi_debug.shape)
    cv2.waitKey(1)
